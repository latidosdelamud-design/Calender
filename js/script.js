document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderCarrusel() {
        ...
        imagenesCarrusel.forEach((img, index) => {
            ...
            slide.className = `carrusel-slide min-w-full h-full relative overflow-hidden ${index === 0 ? 'active' : ''}`;
            slide.innerHTML = `
                <img src="${img.url}" alt="${img.caption}" class="w-full h-full object-cover transition-transform duration-1000">
                <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/95 via-black/50 to-transparent p-8 pt-20">
                    <p class="text-xl md:text-2xl font-medium text-[#f6e0b5] font-serif tracking-wide drop-shadow-md">${img.caption}</p>
                </div>
            `;
            carruselSlides.appendChild(slide);

            const indicador = document.createElement('button');
            indicador.className = `h-2.5 rounded-full transition-all duration-400 ${index === 0 ? 'w-8 bg-[#d4af37] shadow-[0_0_12px_rgba(212,175,55,0.6)]' : 'w-2.5 bg-white/40 hover:bg-white/70'}`;
            indicador.setAttribute('aria-label', `Ir a diapositiva ${index + 1}`);
    ...
    function goToSlide(index) {
        ...
        const allSlides = carruselSlides.querySelectorAll('.carrusel-slide');
    ...
    function updateIndicadores() {
        ...
        indicadores.forEach((ind, i) => {
            if (i === currentSlide) {
                ind.className = 'w-8 h-2.5 rounded-full bg-[#d4af37] shadow-[0_0_12px_rgba(212,175,55,0.6)] transition-all duration-400';
            } else {
                ind.className = 'w-2.5 h-2.5 rounded-full bg-white/40 hover:bg-white/70 transition-all duration-300';
            }
        });
    }
    ...
    function renderCalendario() {
        ...
        calendarioGrid.classList.remove('month-fade-enter');
        void calendarioGrid.offsetWidth;
        calendarioGrid.classList.add('month-fade-enter');
    ...
    if (hasEvent) {
        dayElement.innerHTML = `<span>${dia}</span><span class="event-beacon"></span>`;
    ...
    // Control de reproducción individual en podcast
    const podcastAudios = document.querySelectorAll('.podcast-audio');
    podcastAudios.forEach(audio => {
        audio.addEventListener('play', () => {
            podcastAudios.forEach(otro => {
